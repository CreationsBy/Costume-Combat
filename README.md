...
The 49 production videos in `animations/` have been cropped, resized to their real on-screen resolution, encoded as browser-compatible H.264, and marked for fast-start streaming. Together they remain small enough for a normal GitHub Pages repository; the largest file is about **0.38 MB**.
...
- Recording-side labels are resolved opposite the in-game facing direction, keeping both fighters turned toward one another. Playback speed is normalized by animation type, and attack/reaction clips skip their inactive lead-in frames so input and hit feedback begin immediately while hit and audio cues stay locked to normalized frames.
- The 49 deployment clips are prefetched with five-request concurrency and served from a small in-memory blob cache. Each fighter keeps at most ten live video elements, common combat clips are decoder-warmed during the round intro, and every matching reaction is prepared as soon as its attack starts. Stalled loads are retried, and reused clips wait for their seek-to-start to finish before hit/end logic resumes.
- Retreating uses dedicated reversed walking files rather than repeatedly seeking an MP4 backward, so forward and backward movement remain smooth in Chrome, Edge, and GitHub Pages deployments.
