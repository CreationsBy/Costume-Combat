...
- Recording-side labels are resolved opposite the in-game facing direction, keeping both fighters turned toward one another. Playback speed is normalized by animation type so walking, crouching, attacks, reactions, and cinematics recover quickly while hit and audio cues stay locked to normalized frames.
- The 49 deployment clips are prefetched with five-request concurrency and served from a small in-memory blob cache. Each fighter keeps at most eight live video elements, stalled loads are retried, and reused clips wait for their seek-to-start to finish before hit/end logic resumes.
